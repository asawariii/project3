...
deleteKey.addEventListener("click", () => {
    let initailValue = userInput.value;
    let updatedValue = initailValue.substring(0, initailValue.lenght - 1);
    ...
    key.addEventListener("click", (event)=>{
        ...
        if("+-x/".includes(value)){
            if(lastKeyIsOperator){
                let initailValue= userInput.value;
                let updatedValue= initailValue.substring(0,initailValue.length-1)+value;
                userInput.value=updatedValue;
                return;
            }
